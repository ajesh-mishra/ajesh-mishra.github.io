import React from "react";

export default function ArticleHeader({ postMetadata }) {
  return (
    <>
      <div>
        <h1 className="text-fluent-50 text-2xl sm:text-3xl">
          {postMetadata.title}
        </h1>

        <div className="my-4 text-xs flex space-x-2 place-items-center text-fluent-120">
          <svg
            className="w-4 h-4 fill-fluent-120"
            xmlns="http://www.w3.org/2000/svg"
            viewBox="0 0 512 512"
          >
            <path d="M472.8 168.4C525.1 221.4 525.1 306.6 472.8 359.6L360.8 472.9C351.5 482.3 336.3 482.4 326.9 473.1C317.4 463.8 317.4 448.6 326.7 439.1L438.6 325.9C472.5 291.6 472.5 236.4 438.6 202.1L310.9 72.87C301.5 63.44 301.6 48.25 311.1 38.93C320.5 29.61 335.7 29.7 344.1 39.13L472.8 168.4zM.0003 229.5V80C.0003 53.49 21.49 32 48 32H197.5C214.5 32 230.7 38.74 242.7 50.75L410.7 218.7C435.7 243.7 435.7 284.3 410.7 309.3L277.3 442.7C252.3 467.7 211.7 467.7 186.7 442.7L18.75 274.7C6.743 262.7 0 246.5 0 229.5L.0003 229.5zM112 112C94.33 112 80 126.3 80 144C80 161.7 94.33 176 112 176C129.7 176 144 161.7 144 144C144 126.3 129.7 112 112 112z" />
          </svg>
          <span>tags: </span>
          {postMetadata.tags.map((tag) => {
            return (
              <span key={tag} className="bg-fluent-200 text-fluent-120 py-0.4 px-3 ring-1 ring-fluent-190 rounded-sm">
                {tag}
              </span>
            );
          })}
        </div>

        <p className="text-sm text-fluent-100 sm:text-base">
          {postMetadata.description}
        </p>
      </div>

      <div className="my-6 flex space-x-4 ml-2">
        <div className="relative w-[50px] h-[50px] rounded-full overflow-hidden">
          <img
            src="/1.png"
            alt="author_image"
            className="absolute inset-0 object-cover w-full h-full"
          />
        </div>

        <div>
          <span className="text-sm font-medium text-fluent-70 sm:text-base">
            {postMetadata.author}
          </span>
          <span className="flex place-items-center space-x-2 text-xs text-fluent-100 sm:text-sm">
            <span>{postMetadata.date}</span>
            <svg
              className="w-1 h-1 fill-fluent-100"
              xmlns="http://www.w3.org/2000/svg"
              viewBox="0 0 512 512"
            >
              <path d="M512 256C512 397.4 397.4 512 256 512C114.6 512 0 397.4 0 256C0 114.6 114.6 0 256 0C397.4 0 512 114.6 512 256z" />
            </svg>
            <span>{postMetadata.duration}</span>
          </span>
        </div>
      </div>
    </>
  );
}
